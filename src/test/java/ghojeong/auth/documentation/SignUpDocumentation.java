...
final class SignUpDocumentation extends Documentation {
    private static final String path = "/v1/signup";
    private static final String identifier = "signup";
    ...
    private RequestFieldsSnippet createRequestSnippet() {
        return requestFields(
                ...
                fieldWithPath("password").type(STRING).description("유저의 비밀번호"),
                fieldWithPath("email").optional().type(STRING).description("유저의 이메일"),
    ...
    @DisplayName("POST /v1/signup 문서화")
    @Test
    void test() {
        ...
        ).body(AuthFixture.createSignUpRequest()).when().post(path)
